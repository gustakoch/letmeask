...
export function Room() {
    ...
    useEffect(() => {
        async function verifyIfRoomIsClosed() {
            const roomRef = await database.ref(`rooms/${roomId}`)

            roomRef.on('value', room => {
                const closedAt = room.val().closedAt
    ...
    async function handleNewQuestion(event: FormEvent) {
        ...
        if (newQuestion.trim() === '') {
            return toastErrorAlert('Uma pergunta deve ser informada.')
        }

        if (!user){
            return toastErrorAlert('Você deve fazer login na plataforma para enviar perguntas.')
        }
    ...
    async function handleLoginWithGoogle(event: FormEvent) {
        event.preventDefault()
